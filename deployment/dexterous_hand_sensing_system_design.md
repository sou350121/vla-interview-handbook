...
## 3.5 目标升级：超越人类能力的 DexHand Sensing（定义“超能力”而不是堆传感器）

你要的是“超越人类能力”的灵巧手 sensing，这个目标可以被非常具体地拆成 6 类“超能力”，每一类都对应清晰的传感组合与 KPI 取舍：

### A) 超人类带宽（Human 的皮肤-神经系统做不到 1kHz 全面可观测）

- **目标**：在接触瞬间（10–50ms）就能判断 *滑移趋势 / 卡死 / 约束反力*，并完成反射与微调。
- **传感建议（高频闭环必须依赖）**：
  - 全关节 `motor_current/joint_effort`（500–1000Hz）
  - 指尖压力/剪切阵列（100–200Hz；控制层主力）
  - 手腕 F/T（>=200Hz，若要做拧/撬/推拉门，建议更高）

### B) 超人类动态范围（从“羽毛级轻触”到“工业级对抗”）

- **目标**：同一只手既能夹纸不皱，又能高力矩旋拧/撬动；并且在强对抗中不损坏自身与物体。
- **关键**：需要把“力”分成可控的两件事：**法向力（夹持/按压）**与**切向力（推/拉/摩擦）**，并实时估计它们接近摩擦边界的风险（`slip_risk`）。

### C) 超人类可解释性（知道“物理原因”，而不是只知道成功/失败）

人类很多时候是“感觉”在滑，但说不清原因。超越人类的系统应该能输出可解释的状态：

- `contact_mode`：sticking/sliding/rolling/pivot
- `wrench_est`：当前对物体施加的 \(w=[f,\tau]\)
- `constraint_state`：是否在与桌面/门轴/卡扣对抗
- `failure_mode`：slip/jam/misaligned/occlusion/…

> 这类可解释信号反过来会提升 RM-in-loop 的价值：RM 不再只是打分，而是指导“该换哪种原语”。

### D) 超人类接触几何理解（人类靠皮肤褶皱与经验，机器人可用“更强的几何观测”）

这里的核心不是“每根手指都上视触觉”，而是：
- 用 **少量高信息的几何传感**（1 指视触觉或高分辨阵列）去“理解接触”
- 再把它蒸馏成 **可规模化部署** 的快信号（剪切阵列 + effort/current）

这条路线的 KPI 最优点通常是：
- **1 个视触觉（老师）** + **2–3 个剪切/压力指尖（工兵）** + **全关节 effort/current（反射）**

### E) 超人类自标注与数据飞轮（人类示教昂贵，而系统能自己生成高价值数据）

要超越人类，关键不是只“感知更强”，而是能**把部署自动变成训练**：
- 自动切段（streaming → episode/subtask）
- 自动打标签（primitive/contact_mode/slip_risk）
- 自动挑 hard cases（可恢复边界态）
- 自动触发恢复策略（减少人工介入）

这就是 RM-in-loop 的终极形态：用“物理因果信号”让数据闭环规模化。

### F) 超人类鲁棒性（长期一致性：漂移、磨损、老化都能被系统吸收）

人类皮肤会愈合，但机器人传感器会老化。超越人类的系统必须把“漂移”当成可观测变量：
- 触觉零点漂移、弹性退化 → `sensor_health`
- 电机温升导致的扭矩常数变化 → 热补偿 + 降额曲线
- 线缆/信号偶发丢包 → 多源冗余（effort/current 兜底）
