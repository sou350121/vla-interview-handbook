...
*   **硬件要求**：
    *   **液压驱动**：如 Sanctuary Phoenix，指尖力达 100N，是大载荷任务的首选。
    *   **可折叠掌骨**：如 LEAP V2 的掌骨关节，能增加有效包络半径，通过手掌辅助支撑。
    *   **算法**：关注多指协同的接触稳定性预测，防止因重力矩导致的物体滑移。

#### C. “开可乐”等精细协同 (Complex Tool Use)
*   **挑战**：**爆发力与多点耦合**。拉开易拉罐环需要瞬间的高强度点对点拉力（Tab pulling），同时另一侧手指需稳固锁定瓶身。
*   **硬件要求**：
    *   **高带宽控制**：要求硬件能瞬间响应力矩突变。
    *   **鲁棒齿轮箱**：直驱方案中的行星减速器需具备高抗冲击性。
    *   **算法**：典型的多阶段长程任务，通常需要 **Hierarchical VLA** 或 **CoT** 来拆解“按压-勾取-拉开”的子目标。

#### D. 抓取手机等光滑薄片 (Flat & Smooth Objects)
*   **挑战**：**摩擦力管理与深度估计误差**。手机表面极滑且薄，单指很难从桌面扣起。
*   **硬件要求**：
