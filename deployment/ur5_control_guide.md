...
### 2.3 核心概念：RTDE 与低延迟控制 (Interview Essential)

在面试中谈到 UR 机器人控制时，**RTDE** 是必须要懂的底层词汇：

*   **什么是 RTDE？**
    RTDE (Real-Time Data Exchange) 是 Universal Robots 提供的一种专用通讯接口。它允许外部电脑以**固定周期**（如 2ms/125Hz 或 500Hz）与机械臂进行数据交换。
    *   **读**: 实时读取关节角度、末端位姿、电流、保护性停止状态等。
    *   **写**: 下发目标关节位置 (servoJ)、速度 (speedJ) 或数字 IO 指令。
*   **什么是“低延迟控制”？**
    指的是将外部计算出的控制目标（如 VLA 模型的输出或遥操作指令）以**极短的延迟**、**高频率**同步到机器人控制器。
    *   **直观感受**: 延迟低，机械臂就像“贴”在手上一樣实时跟隨；延迟高，机械臂会产生滞后感、动作一卡一卡。
*   **为什么遥操作 (Teleoperation) 必须用 RTDE？**
    1.  **高频更新**: 遥操作输入变化极快，RTDE 支持 125Hz-500Hz 更新，能显著减少卡顿感。
    2.  **轨迹平滑**: 通常配合**线性插值 (Interpolation)** 或**硬件滤波**，将低频输入（如 30Hz 的 MediaPipe）补齐为高频命令流，让运动丝滑。
    3.  **确定性**: 相比于普通的 Socket 或是 Polyscope 脚本，RTDE 提供了更稳定的时间戳同步，防止控制循环抖动。

> **一句版总结**: RTDE 低延迟控制 = 利用 UR 的 RTDE 接口，以高频、短延迟将外部控制目标同步到硬件，实现机械臂实时、平滑的运动跟随。
...
### 4.4 长时间运行发热导致抖动 (Thermal Jitter)

**现象描述**:
机器人连续运行数小时（特别是执行高频 ServoJ 或重负载任务）后，关节电机温度升高，末端出现细微的高频震颤，或者在低速直线运动时有明显的“卡顿”感，甚至可能触发 `Joint Violation` 报错。

**原因推断**:
1. **控制增益过敏感**: 系统在高增益（High Gain）下运行，当电机发热导致内部润滑脂变稀、机械阻尼减小时，原有的控制参数会变得过冲（Overshoot）。
2. **编码器热漂移**: 高温可能干扰关节内置编码器的反馈精度，导致闭环控制在目标点附近产生微小震荡。
3. **热保护限流**: 控制器为保护电机，可能会调整 PWM 频率或限流，导致力矩输出稳定性下降。
...
- **实时温控监控**: 在代码中增加温度预警逻辑。
  ...
  if max(joint_temps) > 55.0:
      print(f"Warning: Joint temp high {max(joint_temps)}°C, reducing speed.")
  ```
- **动态调整增益**: 当检测到温度升高时，适当**降低 `servoJ` 的 `gain`**（例如从 300 降至 150-200），并稍微**增加 `lookahead_time`** 以平滑轨迹。
- **强制冷却周期**: 在 VLA 模型测试中，建议每运行 45-60 分钟，强制机器人回到 Home 点“休息” 5 分钟（保持抱闸状态），避免热量累积。

> [!CAUTION]
> ### ⚠️ 数据采集一致性警告 (Data Collection Consistency)
> 在进行 **数据采集（Teleoperation/Data Collection）** 时，**严禁动态调整 `gain` 和 `lookahead_time`**！
> *   **后果**: 如果数据集中混合了不同动态特性的动作（有的快响应，有的慢响应），VLA 模型会学到不一致的行为分布，导致训练难以收敛或推理时产生剧烈震荡。
> *   **正确做法**: 
>     1. 选定一组最优参数（如 `gain=300`, `lookahead=0.1`）后，在整个采集周期内保持不变。
>     2. 如果机器人发热导致抖动无法继续，**必须停止采集并强制冷却**，而不是通过降低增益来“带病工作”。
>     3. 始终确保训练时的 `servoJ` 参数与推理（Inference）时完全一致。

- **检查 Payload**: 确保 `setPayload` 包含夹爪及物体的真实重心（CoG）。不准确的重心会导致电机持续输出额外扭矩以抵消重力，从而快速发热。
