...
- **[相机标定与手眼对齐 (Camera Calibration)](./camera_calibration_eye_in_hand.md)**: Eye-in-Hand vs Eye-to-Hand 標定實戰。
...
- **[灵巧手通讯与部署實戰 (DexHand Communication)](./dexterous_hand_communication_deployment.md)**: 通讯架构 (CANFD, EtherCAT)、Retargeting 與線纜管理。
...
- **[具身智能数据采集概览 (Embodied Data Collection)](./embodied_data_collection_overview.md)**: POV 第一视角 (EgoScale)、Sim2Real 規模化與真機 RL。
...
- **工程党**: 重点研读 **[VLA 边缘部署](./vla_model_edge_deployment.md)** 与 **[多模態同步](./multimodal_data_synchronization.md)**。
- **算法党**: **[Sim-to-Real 遷移策略](./sim_to_real_transfer_strategies.md)** 与 **[數據採集](./dexterous_hand_data_collection.md)** 是核心重點。
