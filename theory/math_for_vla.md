...
### 1.4 特征值与特征向量（Eigenvalues & Eigenvectors）
- **公式**：
  $$Av = \lambda v$$
- **变量定义**：`A` (方阵), `v` (特征向量), `λ` (特征值)。
- **物理意义**：它描述了矩阵 `A` 在哪个方向（`v`）上只进行缩放（`λ`）而不改变方向。这在分析动力系统稳定性和数据降维（PCA）中极度重要。
- **公式大白话**：
  - **给定** 一个变换（矩阵 `A`）。
  - **翻译**：找出那些“最有骨气”的向量 `v`，被变换之后，它们只是变长或变短了，但完全没歪。`λ` 就是变长了多少倍。

### 1.5 奇异值分解（SVD）
- **公式**：
  $$A = U \Sigma V^\top$$
- **变量定义**：`U, V` (旋转/正交矩阵), `Σ` (对角矩阵，包含奇异值)。
- **物理意义**：任何复杂的矩阵变换，都可以拆解为“旋转 + 缩放 + 旋转”。这是模型压缩、噪声滤除（低阶近似）的数学终点。
- **公式大白话**：
  - **给定** 一个杂乱无章的数据矩阵 `A`。
  - **翻译**：把它拆成三个零件。`Σ` 里的奇异值会告诉你哪些特征是“大头”，哪些是“噪音”。你可以扔掉小的奇异值，实现无损或近无损的压缩。

### 1.6 范数（Norms: L1, L2）
- **公式**：
  $$\|x\|_1 = \sum |x_i|, \quad \|x\|_2 = \sqrt{\sum x_i^2}$$
- **变量定义**：`x` (特征或权重向量)。
- **物理意义**：衡量向量的“长度”或“大小”。L1 倾向于产生稀疏性（让很多项变 0），L2 倾向于让每一项都变得很小（平滑）。
- **公式大白话**：
  - **给定** 一个向量（比如误差或权重）。
  - **翻译**：计算它有多“重”。L1 是走方格路的距离（曼哈顿），L2 是拉直线的距离（欧几里得）。在 Loss 里加 L2 就像在给模型减重，防止它长出赘肉（过拟合）。

### 1.7 正交投影（Orthogonal Projection）
- **公式**：
  $$\text{proj}_d(x) = \frac{x \cdot d}{\|d\|^2} d$$
- **变量定义**：`x` (原始向量), `d` (目标方向)。
- **物理意义**：将一个向量投影到一个特定的子空间或方向上。
- **公式大白话**：
  - **给定** 一个乱动的向量 `x`。
  - **翻译**：看看这个向量在 `d` 这个方向上到底占了多少分量。就像在墙角打个手电筒，看 `x` 在地上的影子。这在安全控制中很有用，比如把不安全的动作方向给过滤掉。
- **ASCII 描述系统（投影过滤）**：
```text
          x (原始意图)
         / |
...
### 2.3 梯度场（Gradient Field）
- **公式**：
  $$\nabla \mathcal{L} = \left[ \frac{\partial \mathcal{L}}{\partial w_1}, \frac{\partial \mathcal{L}}{\partial w_2}, \dots, \frac{\partial \mathcal{L}}{\partial w_n} \right]^\top$$
- **变量定义**：`L` (标量损失值), `w` (参数向量), `∇L` (梯度向量)。
- **物理意义**：它是一个指向“最陡峭上升方向”的向量。梯度场则是空间中每一点梯度的集合。
- **公式大白话**：
  - **给定** 一个复杂的山坡（损失函数的地形）。
  - **翻译**：在山坡的任何一个位置，梯度向量都会告诉你：“往这个方向走，海拔升得最快”。在训练时，我们要反着它走（梯度下降），即往坡度降得最快的方向溜。
- **ASCII 描述系统（流场直觉）**：
```text
    损失地形 (等高线):
       (  (  .  )  )   <--- 最小值 (谷底)
...
### 2.4 方向导数（Directional Derivative）
- **公式**：
  $$D_u \mathcal{L} = \nabla \mathcal{L} \cdot u, \quad \|u\|=1$$
- **变量定义**：`u` (你想探索的方向单位向量), `∇L` (梯度), `DuL` (该方向的变化率)。
- **物理意义**：衡量如果你沿着某个特定的方向 `u` 挪一小步，损失值会变多少。
- **公式大白话**：
  - **给定** 一个前进方向。
  - **翻译**：把你选的方向和最陡的方向（梯度）做个对比（点积）。如果你顺着梯度走，变化最大；如果你横着走（垂直），变化就是 0。

### 2.5 海森矩阵（Hessian Matrix）
- **公式**：
  $$H = \nabla^2 \mathcal{L}, \quad H_{ij} = \frac{\partial^2 \mathcal{L}}{\partial w_i \partial w_j}$$
- **变量定义**：`H` (n x n 的二阶导数矩阵)。
- **物理意义**：它描述了损失函数的**曲率（Curvature）**。
- **公式大白话**：
  - **给定** 此时此刻的坡度（一阶导数）。
  - **翻译**：梯度告诉你坡往哪斜，海森矩阵告诉你这个坡是平缓的弧形，还是极其陡峭的折点。如果海森矩阵的值很大，说明地形变化剧烈，学习率就要设小一点，否则容易飞出去。
