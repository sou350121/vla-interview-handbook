...
- **内积与相似度（Dot Product）**：
  - **公式**：
    ...
    $$
    \text{sim}(A, B) = A \cdot B = \sum A_i B_i = \|A\| \|B\| \cos(\theta)
    $$
...
- **矩阵乘法（Linear Layer）**：
  ...
  - **公式大白话**：**给定**特征 `x`，**翻译**为通过权重 `W` 重新组合，把信息 from “视觉空间”搬到“动作空间”。
- **低秩分解（LoRA）**：
  - **公式**：
...
- **特征值与奇异值（Eigen / SVD）**：
  - **公式**：
    ...
    $$
    Av = \lambda v, \quad A = U \Sigma V^\top
    $$
...
- **范数与正交投影（Norms & Projection）**：
  - **公式**：
    ...
    $$
    \|x\|_2 = \sqrt{\sum x_i^2}, \quad \text{proj}_d(x) = \frac{x \cdot d}{\|d\|^2} d
    $$
...
- **扩散模型与流匹配（Diffusion & Flow Matching）**：
  - **公式**：
    ...
    $$
    x_t = \sqrt{\bar{\alpha}_t} x_0 + \sqrt{1 - \bar{\alpha}_t} \epsilon, \quad v_t = x_1 - x_0
    $$
