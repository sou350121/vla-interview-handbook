...
## 1.1 产业观点补充：具身模型 ≠ “把 LLM 塞进机器人”

> 这部分总结自变量在公开表达中反复强调的一点：**具身智能不是语言/多模态模型的下游应用**，而是一个更“物理世界原生”的基础模型范式。  
> 参考（访谈/报道汇总）：[搜狐相关报道](https://www.sohu.com/a/922941858_114765)、[盖世汽车相关报道](https://auto.gasgoo.com/news/202503/25I70421301C106.shtml)。

### A) 物理世界的信息，很多是“看不出来但会影响行为”的

- **核心难点**：仅靠静态视觉，很多关键差异在当下不可见，但会在后续交互里“爆雷”（例如：瓶盖是否拧紧，视觉差异很小，但翻转/倒水才出现漏不漏的结果）。
- **对 VLA 的含义**：模型需要把 **感知 → 动作 → 后果** 串成**长时序因果链**，而不是只拟合单帧输入输出。

### B) 长时序 + 未来推演：manipulation 不只是“反应”，还要“预判”

- **现实任务**往往要求机器人在行动前进行某种“推演”：比如倒水前判断会不会漏、整理桌面时决定先拿什么再放什么。
- **对模型设计的含义**：需要更强的 **世界模型/未来预测** 成分，而不只是把 policy 当作动作回归器。

### C) 损失函数从“预测动作”升级为“预测多模态状态”

一个关键主张是：如果只做动作预测，模型容易退化成“模仿者”；更能逼近物理规律的目标是让模型学习：
- “我做了这个动作之后，**下一秒/未来**的视觉会怎么变？”
- “接触发生后，触觉/力觉会如何变化（或消失）？”

这可以理解为把训练目标从 **action prediction** 扩展到 **multimodal state prediction / world modeling**，从而把因果结构压进模型里（同样来自公开报道表述的归纳：[搜狐相关报道](https://www.sohu.com/a/922941858_114765)、[盖世汽车相关报道](https://auto.gasgoo.com/news/202503/25I70421301C106.shtml)）。

### D) WALL-A vs WALL-OSS（读文时别混淆）

- **WALL-A**：更像“主模型/旗舰操作模型”的对外叙事载体（强调物理世界基础模型、长时序操作与泛化）。
- **WALL-OSS**：开源版本/开源系统，更强调工程可复现与生态协作（把完整链路跑通，让社区能复现与迭代）。
