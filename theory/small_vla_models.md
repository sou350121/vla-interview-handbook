...
> **最后更新**: 2025-12-08
> **包含**: 2025 年最新模型 (Evo-1, SmolVLA, ControlVLA, Eva-VLA)
...
## 🔬 现有小模型 VLA 方案 (2025 最新)

### 2.0 2025 年小模型 VLA 速览
...
│   模型           参数量      发布时间    核心创新                │
│   ─────────────────────────────────────────────────────────────  │
│   Evo-1          770M       2025.11    跨调制扩散Transformer    │
│   SmolVLA        450M       2025.06    Flow Matching + 异步推理 │
│   ControlVLA     -          2025.06    ControlNet 少样本适配    │
│   Eva-VLA        -          2025.09    鲁棒性评估框架           │
│                                                                 │
│   对比基线:                                                      │
│   OpenVLA        7B         2024.06    开源 VLA 标杆            │
│   π0             3B         2024.10    Flow Matching 先驱       │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

| 模型 | 参数量 | LIBERO 成功率 | 推理速度 | 发布 |
|:---|---:|---:|---:|:---|
| **Evo-1** | 770M | **94.8%** | 快 | 2025.11 |
| **SmolVLA** | 450M | ~85% | 45-60 Hz | 2025.06 |
| **ControlVLA** | - | 76.7% (10-shot) | - | 2025.06 |
| OpenVLA | 7B | ~80% | ~5 Hz | 2024.06 |
| π0 | 3B | ~88% | ~15 Hz | 2024.10 |

---

### 2.1 Evo-1 (2025.11) 🆕

> **论文**: [Evo-1: Towards Unified Vision-Language-Action Model with Cross-Modal Diffusion](https://arxiv.org/abs/2511.04555)
> **参数量**: 770M
> **LIBERO 成功率**: 94.8%
...
│                     │                                           │
│                     ▼                                           │
│   Text ───▶ [Cross-Modulation] ◀── 创新: 跨调制注意力           │
│                     │                                           │
│                     ▼                                           │
│              [Diffusion Transformer]                            │
│                     │         (轻量级扩散模块)                   │
│                     ▼                                           │
│              Action Output ──▶ 7-DoF Actions                   │
│                                                                 │
│   总参数: ~770M                                                  │
│   训练策略: 两阶段 (感知对齐 → 动作学习)                         │
└─────────────────────────────────────────────────────────────────┘
...
1. **Cross-Modulation Diffusion Transformer (CMD-T)**
   - 跨模态调制注意力：视觉-语言特征深度融合
   - 比传统 Cross-Attention 更轻量

2. **两阶段训练**
   - Stage 1: 感知对齐（保留 VLM 表示能力）
   - Stage 2: 动作学习（端到端微调）

3. **性能突破**
   - LIBERO: **94.8%** (超越所有大模型)
   - Meta-World: 超越前 SOTA 12.4%
   - RoboTwin: 超越前 SOTA 6.9%

---

### 2.2 SmolVLA (Hugging Face, 2025.06) 🆕

> **论文**: [SmolVLA: A Compact Open-Source VLA Model](https://arxiv.org/abs/2506.01844)
> **参数量**: 450M
> **推理速度**: 45-60 Hz (RTX 4070)
...
### 2.3 ControlVLA (2025.06) 🆕

> **论文**: [ControlVLA: Object-Centric Few-Shot Adaptation of VLA Models](https://arxiv.org/abs/2506.16211)
> **核心创新**: ControlNet 风格的少样本适配

**10-20 次示范达到 76.7% 成功率**
...
> **论文**: [Eva-VLA: Evaluating VLA Robustness under Physical Variations](https://arxiv.org/abs/2509.18953)
> **核心贡献**: VLA 鲁棒性评估框架
...
### 2.5 OpenVLA + 量化 (Stanford, 2024) [基线]
...
│                                                                 │
│   中型 (500M-3B)                                                 │
│   ├── Evo-1 (770M): 🆕 2025 SOTA，LIBERO 94.8%                  │
│   └── SmolVLA-2B: 消费级 GPU 可训练                              │
│                                                                 │
│   小型 (<500M)          ◀── 2025 研究热点!                       │
│   ├── SmolVLA (450M): 🆕 Flow Matching + 异步推理               │
│   ├── SmolVLA-210M: 边缘可部署，超越 55B RT-2                   │
│   └── 未来方向: 100M 级别?                                       │
│                                                                 │
│   高效适配 (不改变模型大小)                                       │
│   ├── ControlVLA: 🆕 10-shot 达到 76.7%                         │
│   └── VLA-Adapter: LoRA 减少 70% 可训练参数                      │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
| 模型 | 参数量 | LIBERO | 推理速度 | 边缘部署 | 发布 |
|:---|---:|---:|---:|:---:|:---|
| RT-2 | 55B | ~85% | ~1 Hz | ❌ | 2023 |
| OpenVLA | 7B | ~80% | ~5 Hz | ⚠️ 量化后 | 2024.06 |
| π0 | 3B | ~88% | ~15 Hz | ⚠️ | 2024.10 |
| **Evo-1** | **770M** | **94.8%** | 快 | ✅ | 2025.11 |
| **SmolVLA** | **450M** | ~85% | **45-60 Hz** | ✅ | 2025.06 |
| SmolVLA-mini | 210M | ~80% | 60+ Hz | ✅ | 2025.06 |

**关键发现**: Evo-1 (770M) 以 1.4% 的参数量超越 RT-2 (55B) 的性能!
