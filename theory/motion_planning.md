...
- **价值函数引导采样**: 训练 value network 评估节点好坏，提高采样效率。
...
- **Pilz**: MoveIt 内置笛卡尔规划器，适合直线/插补。
- **TrajOpt / STOMP 插件**: 需要额外安装，提供优化型规划。

### 4.2 NVIDIA cuRobo

- 基于 GPU 的多规划器并行，典型流程：随机种子 → IK 候选 → TrajOpt。
- 在 Jetson Orin 上可实现 < 100 ms 的 7DoF 机械臂规划。
...
si.setStateValidityChecker(ob.StateValidityCheckerFn(lambda s: is_state_valid(si, s)))
si.setup()
...
### Q1: 采样式规划 vs 轨迹优化？
- 采样式：易找到可行解，适合复杂障碍，但路径需要平滑。
- 轨迹优化：可直接考虑动力学和成本，但对初值敏感。
- 工程：RRT → TrajOpt 的两阶段 Pipeline。
...
### Q3: 高维机械臂如何加速规划？
- 限定采样边界，利用 IK 初值。
- 任务空间采样 (Task-Space RRT)。
- GPU 并行 (cuRobo)。

### Q4: CHOMP 为什么需要障碍势能？
- 通过距离场定义势能，沿梯度方向推开轨迹，保证与障碍保持安全距离。

### Q5: 规划结果如何保证可执行？
- 时间参数化 (TOTG)。
- 检查速度/加速度限制。
- 控制器 (PD/MPC) 跟踪，并加入反馈校正。
