...
π0 的架构设计的核心在于将大规模 VLM 的语义理解能力与高效的 Flow Matching 生成头相结合。

### 1.1 系统对比概览 (Component Comparison)
...
π0 的信息流展现了如何从“意图”转化为“物理流”：

```
                    π0 核心架构流图 (VLA-Flow)
┌─────────────────────────────────────────────────────────────┐
│  [输入端]  Image (RGB) + Language Instruction (Task)         │
└───────────────┬─────────────────────────────────────────────┘
                │
                ▼
┌──────────────────────────────────────┐  [语义编码层: Backbone]
...
│ │  x_t (当前带噪动作/中间状态)      │ │
│ ├──────────────────────────────────┤ │  <-- 输入当前位置
│ │  t (时间进度：1.0 -> 0.0)         │ │  <-- 输入推理进度
│ ├──────────────────────────────────┤ │
│ │  v_theta = predict_velocity(...)  │ │  <-- 预测最优指向箭头
│ └──────────────────────────────────┘ │
└───────────────┬──────────────────────┘
                │
                ▼
...
1.  **开始 ($t=0$)**: $x_0 = [0, 0]$。
2.  **中间 ($t=0.5$)**: $x_{0.5} = 0.5 \times [0, 0] + 0.5 \times [10, 10] = [5, 5]$。
3.  **结束 ($t=1$)**: $x_1 = [10, 10]$。

**物理直觉**: 无论什么时候，物体都在这条连接 $[0,0]$ 和 $[10,10]$ 的直线上，且匀速移动。
...
**这就是 Flow Matching 的核心：学习这个“指路的速度向量”。**
