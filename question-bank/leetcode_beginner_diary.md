...
## 第14题：搜索插入位置（Search Insert Position）
...
**Q**: 为什么最后返回 `l` 而不是 `mid`？  
**A**: 循环结束时 `l` 指向第一个 ≥ target 的位置，这正是插入位置。

**Q**: 能用 `l < r` 而不是 `l <= r` 吗？  
**A**: 可以，但返回逻辑会不同。标准写法是 `l <= r`。

**Q**: 时间复杂度为什么是 O(log n)？  
**A**: 每次都减半，n 个元素最多查找 log₂n 次。

**Q**: 如果数组有重复元素怎么办？  
**A**: 这道题保证升序且无重复，但如果有重复，需要找第一个出现的位置。
...
**Q**: 为什么不用哈希表？  
**A**: 可以用，但空间 O(n)。双指针法只要 O(1)。

**Q**: 如果没有交点会怎样？  
**A**: 两个指针最终都会走到 None，相等时返回 None。

**Q**: 时间复杂度是多少？  
**A**: O(m + n) - 每个链表最多走两遍。

**Q**: 能改链表结构吗？  
**A**: 题目不允许，而且实际场景中链表可能被其他代码使用。
...
## 第33题：二叉树的中序遍历（Binary Tree Inorder Traversal）
...
好，小白模式继续。  
**这题是二叉树遍历的"第一题"，非常非常重要。**

我会按顺序来：**人话 → 名词 → 思路 → 代码（两种）**。

---

### 这题在干嘛？（一句话）

**按"左 → 根 → 右"的顺序，把树里的数字取出来。**

这就叫：**中序遍历（Inorder Traversal）**
...
**Q**: 递归和迭代哪个更好？  
**A**: 递归更好理解，迭代更省空间（避免递归栈溢出）。

**Q**: 时间复杂度是多少？  
**A**: O(n) - 每个节点访问一次。

**Q**: 空间复杂度呢？  
**A**: 递归 O(h)（h是树高），迭代也是 O(h)（栈的深度）。

---

## 第33题补充：层序遍历（Binary Tree Level Order Traversal）

_【待补充 - 这是另一种遍历方式，按层从上到下】_
